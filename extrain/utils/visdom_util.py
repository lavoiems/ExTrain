...
def save_tsne(X, labels, colormap, out_file, env='main', id=0, labels_name=None, title=''):
    logger.info('Saving tsne')
    plt.clf()
    if len(labels):
        ...
        visualizer.scatter(
            X.transpose(1,0), win='scatter%s' % id, Y=labels+1, env=env,
            opts={'legend': labels_name, 'title': title})
    else:
        plt.scatter(*X)
        visualizer.scatter(
            X.transpose(1,0), win='scatter%s' % id, env=env, opts={'title': title})
    plt.savefig(out_file)
    logger.info('Done saving tsne')


def save_train_error(errors, out_file, env='main', id=0, title=''):
    logger.info('Saving training error')
    plt.clf()
    plt.plot(errors)
    plt.title(title)
    plt.savefig(out_file)
